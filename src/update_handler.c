...
static gboolean img_to_boot_emmc_handler(RaucImage *image, RaucSlot *dest_slot, const gchar *hook_name, GError **error)
{
	...
	g_debug("Toggling active eMMC boot partition %sboot%d -> %s", realdev, part_active,
			part_slot->device);
	if (!r_emmc_activate_bootpart(realdev, INACTIVE_BOOT_PARTITION(part_active), &ierror)) {
		g_propagate_error(error, ierror);
		return FALSE;
	}
...
static gboolean emmc_boot_linked_migration_helper(RaucSlot *dest_slot, GError **error)
{
	...
	GError *ierror = NULL;
	g_autofree gchar *base_device = r_emmc_extract_base_dev(real_dest, &ierror);
	if (base_device == NULL) {
		g_propagate_error(error, ierror);
		return FALSE;
	}
	...
	/* Skip migration if the target slot is not the currently active eMMC boot partition */
	...
	g_debug("Toggling active eMMC boot partition %sboot%d", base_device, INACTIVE_BOOT_PARTITION(active_partition));
	if (!r_emmc_activate_bootpart(base_device, INACTIVE_BOOT_PARTITION(active_partition), &ierror)) {
		g_propagate_error(error, ierror);
		return FALSE;
	}
...
static gboolean img_to_emmc_boot_linked_handler(RaucImage *image, RaucSlot *dest_slot, const gchar *hook_name, GError **error)
{
	...
	if (!emmc_boot_linked_migration_helper(dest_slot, &ierror)) {
		g_propagate_error(error, ierror);
		return FALSE;
	}

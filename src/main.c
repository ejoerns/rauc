...
static gboolean service_start(int argc, char **argv)
{
	...
	if (g_strcmp0(r_context()->system_state->boot_id, r_context()->boot_id) == 0) {
		...
		/* update boot ID */
		g_free(r_context()->system_state->boot_id);
		r_context()->system_state->boot_id = g_strdup(r_context()->boot_id);

		if (!r_system_state_save(&ierror)) {
			g_warning("Failed saving global state: %s", ierror->message);
			g_clear_error(&ierror);
		}
	}

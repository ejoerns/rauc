...
gchar* r_emmc_extract_base_dev(const gchar *device_path, GError **error)
{
	...
	gchar *base_device = r_regex_match_simple("^(/dev/mmcblk[0-9]+)", device_path);
...
gboolean r_emmc_activate_bootpart(const gchar *device, gint activate_partition, GError **error)
{
	...
	g_return_val_if_fail(activate_partition >= 0 && activate_partition < EMMC_BOOT_PARTITIONS, FALSE);
	g_return_val_if_fail(error == NULL || *error == NULL, FALSE);

	if (!r_emmc_write_bootpart(device, activate_partition, &ierror)) {
	...
	if (activate_partition != part_active_after) {
		g_set_error(error, R_UPDATE_ERROR, R_UPDATE_ERROR_FAILED,
				"Activating the boot partition failed! Your kernel is most-likely affected by the ioctl ext_csd bug: see https://rauc.readthedocs.io/en/latest/advanced.html#update-bootloader-in-emmc-boot-partitions");
		return FALSE;
	}

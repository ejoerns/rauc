...
def test_write_emmc_boot_linked_point_to_same_boot_dev(system):
    ...
    out, err, exitcode = run(f"{system.prefix} write-slot bootloader.0 install-content/rootfs.img")
    assert exitcode == 1
    assert "No data directory or status file set, falling back to per-slot status." in err
    assert "Consider setting 'data-directory=<path>' or 'statusfile=<path>/per-slot' explicitly." in err
    assert "Using per-slot statusfile. System status information not supported!" in err
    assert (

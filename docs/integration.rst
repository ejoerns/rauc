...
up properly. This means having two redundant partitions of equal size if you
want to have a simple A/B scenario, or a tiny and a larger partition for a
simple recovery/A setup.

Partitioning the storage you use is part of the bootstrap process and **not**
in the scope of an update tool like RAUC.

Additionally, you may also need to reserve space for your bootloader, boot
state information (such as state backend for barebox or environment partition
for U-Boot), data partition(s) or further.
...
Unlike SD cards, an eMMC is fixed to your board and thus needs to be set up
from a Linux image running on your board.

Note that an eMMC also provides dedicated boot partitions that can be selected
by setting exctCSD registers and thus potentially allow atomic bootloader
updates.

NAND
~~~~

NAND allows you either to be partition by devicetree information or (indirect)
by using UBI that support creating multiple volumes.

		...
		gconstpointer user_data)
{
	...
	fixture->codesign_compat = FALSE;

	prepare_bundle(fixture, user_data);
}

/* Tests compatiblitiy with check purpose 'codesign' but missing 'keyUsage' */
static void bundle_fixture_set_up_bundle_codesign_compat(BundleFixture *fixture,
		gconstpointer user_data)
{
	replace_strdup(&r_context_conf()->certpath, "test/openssl-ca/dev/xku-codeSigning-compat.cert.pem");
	replace_strdup(&r_context_conf()->keypath, "test/openssl-ca/dev/private/xku-codeSigning-compat.pem");
	/* cert is already checked once during signing */
	g_free(r_context()->config->keyring_check_purpose);
	r_context()->config->keyring_check_purpose = g_strdup("codesign-rauc");
...
int main(int argc, char *argv[])
{
	...
	for (RManifestBundleFormat format = R_MANIFEST_FORMAT_PLAIN; format <= R_MANIFEST_FORMAT_VERITY; format++) {
		...
		g_test_add(dup_test_printf(ptrs, "/bundle/purpose/codesign-compat/%s", format_name),
				BundleFixture, bundle_data,
				bundle_fixture_set_up_bundle_codesign_compat, bundle_test_purpose_codesign,
				bundle_fixture_tear_down);

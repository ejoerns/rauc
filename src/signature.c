...
gboolean signature_init(GError **error)
{
	...
	if (!ret) {
		g_set_error(
				...
				"Failed to initialize OpenSSL crypto: %s", ERR_error_string(ERR_get_error(), NULL));
		return FALSE;
	}
	...
	if (!ret) {
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_CRYPTOINIT_FAILED,
				"Failed to configure OpenSSL X509 purpose: %s", ERR_error_string(ERR_get_error(), NULL));
		return FALSE;
	}
...
static ENGINE *get_pkcs11_engine(GError **error)
{
	...
	if (e == NULL) {
		g_set_error(
				...
				"failed to load PKCS11 engine: %s", ERR_error_string(ERR_get_error(), NULL));
	...
	if (env != NULL) {
		if (!ENGINE_ctrl_cmd_string(e, "MODULE_PATH", env, 0)) {
			g_set_error(
					...
					"failed to configure PKCS11 module path: %s", ERR_error_string(ERR_get_error(), NULL));
			goto free;
		}
	}
	...
	if (ENGINE_init(e) == 0) {
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_LOAD_FAILED,
				"failed to initialize PKCS11 engine: %s", ERR_error_string(ERR_get_error(), NULL));
	...
	if (env != NULL && env[0] != '\0') {
		if (!ENGINE_ctrl_cmd_string(e, "PIN", env, 0)) {
			g_set_error(
					...
					R_SIGNATURE_ERROR,
					R_SIGNATURE_ERROR_PARSE_ERROR,
					"failed to configure PKCS11 PIN: %s", ERR_error_string(ERR_get_error(), NULL));
			goto finish;
		}
	}
...
static EVP_PKEY *load_key_file(const gchar *keyfile, GError **error)
{
	...
	if (res == NULL) {
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_PARSE_ERROR,
				"failed to parse key file '%s': %s", keyfile, ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}
out:
	BIO_free_all(key);
...
static EVP_PKEY *load_key_pkcs11(const gchar *url, GError **error)
{
	...
	if (res == NULL) {
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_LOAD_FAILED,
				"failed to load PKCS11 private key for '%s': %s", url, ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}
out:
	return res;
...
static X509 *load_cert_file(const gchar *certfile, GError **error)
{
	...
	if (res == NULL) {
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_PARSE_ERROR,
				"failed to parse cert file '%s': %s", certfile, ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}
...
static X509 *load_cert_pkcs11(const gchar *url, GError **error)
{
	...
	if (!ENGINE_ctrl_cmd(e, "LOAD_CERT_CTRL", 0, &parms, NULL, 0) || (parms.cert == NULL)) {
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_PARSE_ERROR,
				"failed to load PKCS11 certificate for '%s': %s", url, ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}
	res = parms.cert;
...
GBytes *cms_sign(GBytes *content, gboolean detached, const gchar *certfile, const gchar *keyfile, gchar **interfiles, GError **error)
{
	...
	if (cms == NULL) {
		g_set_error(
				...
				"failed to create signature: %s", ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}
...
gboolean cms_verify_bytes(GBytes *content, GBytes *sig, X509_STORE *store, CMS_ContentInfo **cms, GBytes **manifest, GError **error)
{
	...
	if (!verified) {
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_INVALID,
				"signature verification failed: %s", ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}
...
GBytes *cms_encrypt(GBytes *content, gchar **recipients, GError **error)
{
	...
	if (cms == NULL) {
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_INVALID,
				"Failed to encrypt: %s", ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}
	if (!i2d_CMS_bio(outsig, cms)) {
		g_set_error_literal(
...
GBytes *cms_decrypt(GBytes *content, const gchar *certfile, const gchar *keyfile, GError **error)
{
	...
	if (!CMS_decrypt(icms, privkey, decrypt_cert, NULL, outdecrypt, 0)) {
		...
		g_set_error(
				...
				R_SIGNATURE_ERROR,
				R_SIGNATURE_ERROR_INVALID,
				"Failed to decrypt CMS EnvelopedData: %s", ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}

...
for i in $(seq 0 9); do
	i="$(printf "%03d" $i)"
	echo -n "."
	openssl req -x509 -newkey rsa:4096 -nodes -keyout private-key-$i.pem -out cert-$i.pem -subj "/O=Test Org/CN=Test Org Development $i" > /dev/null 2>&1
done
...
for i in $(seq 0 9); do
	...
	openssl req -new -x509 -key private-key-$i.pem -out cert-$i.pem -days 360 -subj "/O=Test Org/CN=Test Org Development $i" > /dev/null
done

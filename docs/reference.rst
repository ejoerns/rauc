...
``statusfile``
  If this key exists, its value points to a file where status information shall
  be stored (e.g. slot specific metadata, see :ref:`slot-status`). It is
  recommended to put the file on a persistent filesystem surviving updates.
...
There are some slot specific metadata that are of interest for RAUC, e.g. a hash
...
to be performed or can be skipped. These slot metadata can be persisted in one
of two ways: either in a slot status file that is created inside each slot which
contains a writable filesystem or in a single, central and comprehensive status
file that lives on a persistent filesystem surviving updates. The former is
RAUC's default whereas the latter mechanism is chosen by making use of the
optional key :ref:`statusfile <statusfile>` in the system.conf file. Both are
formatted as INI-like key/value files where the particular data are grouped in a
section named [slot] for the case of a slot individual file or in sections
termed with the slotname (e.g. [slot.rootfs.1]) for the central status file
case:
...
and ``bundle.build`` are copies of the respective manifest data. More
information can be found in this :ref:`subsection <sec-manifest-update>` of

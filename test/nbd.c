...
static void test_direct_read(NBDFixture *fixture, gconstpointer user_data)
{
	...
	res = start_nbd_server(nbd_srv, &ierror);
	if (!data->err_domain) {
	...
	res = nbd_read(nbd_srv->sock, (guint8*)&magic, sizeof(magic), 0, &ierror);
	g_assert_no_error(ierror);
	g_assert_true(res);
...
static void test_check_invalid_bundle(NBDFixture *fixture, gconstpointer user_data)
{
	...
	res = check_bundle("http://127.0.0.1/test/invalid-sig-bundle.raucb", &bundle, TRUE, NULL, &ierror);
...
static void test_check_not_found(NBDFixture *fixture, gconstpointer user_data)
{
	...
	res = check_bundle("http://127.0.0.1/test/missing-bundle.raucb", &bundle, TRUE, NULL, &ierror);
...
static void test_plain_bundle(NBDFixture *fixture, gconstpointer user_data)
{
	...
	res = check_bundle("http://127.0.0.1/test/good-bundle.raucb", &bundle, TRUE, NULL, &ierror);
	g_assert_error(ierror, R_BUNDLE_ERROR, R_BUNDLE_ERROR_FORMAT);
	g_assert_false(res);
	g_assert_null(bundle);
}
...
static void test_verity_bundle(NBDFixture *fixture, gconstpointer user_data)
{
	...
	res = check_bundle("http://127.0.0.1/test/good-verity-bundle.raucb", &bundle, TRUE, NULL, &ierror);
	g_assert_no_error(ierror);
	g_assert_true(res);
...
static void test_nbd_mount(NBDFixture *fixture, gconstpointer user_data)
{
	...
	res = check_bundle(data->bundle_url, &bundle, TRUE, &data->access_args, &ierror);
	if (!data->err_domain) {
		g_assert_no_error(ierror);
		g_assert_true(res);
		g_assert_nonnull(bundle);

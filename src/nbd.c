...
void free_nbd_device(RaucNBDDevice *nbd_dev)
{
	...
	if (nbd_dev->index_valid) {
		remove_nbd_device(nbd_dev, NULL);
		/* FIXME report error */
	}
...
gboolean setup_nbd_device(RaucNBDDevice *nbd_dev, GError **error)
{
	...
	nl = netlink_connect(&driver_id);
	if (!nl) {
		res = FALSE;
		g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, "failed to connect to netlink");
		goto out;
	}
...
static gboolean prepare_curl(struct RaucNBDTransfer *xfer)
{
	CURLcode code = 0;
...
static gboolean collect_curl_stats(struct RaucNBDContext *ctx, struct RaucNBDTransfer *xfer)
{
	CURLcode code;
	double time;
	curl_off_t size;
...
static gboolean start_read(struct RaucNBDContext *ctx, struct RaucNBDTransfer *xfer)
{
	CURLcode code = 0;
	CURLMcode mcode = 0;
...
static gboolean finish_configure(struct RaucNBDContext *ctx, struct RaucNBDTransfer *xfer)
{
	...
	if (!g_str_has_prefix((gchar*)xfer->buffer, "hsqs")) {
		g_variant_dict_insert(&dict, "error", "s", "invalid bundle header");
		res = FALSE;
		goto reply;
	}
	...
	if (result == NULL) {
		if (err == 0) {
			...
			g_set_error(
					...
					"failed to get user %s from database", user);
			return FALSE;
		}
	}
...
gboolean nbd_read(gint sock, guint8 *data, size_t size, off64_t offset, GError **error)
{
	...
	if (reply.error != GUINT32_TO_BE(0))
		g_error("error in nbd reply header");

...
To introspect an encrypted bundle on your build host, you need to provide
the encryption key via the ``--key`` argument::
...
For being able to install an encrypted RAUC Bundle on the target, you need to
configure the location of the target's private key in the system.conf:
...
.. code-block:: cfg
   ...
   [encryption]
   key=pkcs11:token=rauc;object=privat-key-1

The invocation of the installation then does not differ from the installation
of an unencrypted bundle::

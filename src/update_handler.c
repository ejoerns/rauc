...
static GUnixInputStream* open_file_as_unix_input_stream(const gchar *filename, int *fd, GError **error)
{
	...
	if (fd_out == -1) {
		g_set_error(error, R_UPDATE_ERROR, R_UPDATE_ERROR_FAILED,
				"Opening file %s failed: %s", filename, strerror(errno));
		return NULL;
	}
...
static gboolean splice_with_progress(GUnixInputStream *image_stream,
		GUnixOutputStream *out_stream, goffset image_size, GError **error)
{
	ssize_t out_size = 0;
	goffset sum_size = 0;
		...
		GUnixOutputStream *out_stream, GError **error)
{
	...
	if (!splice_with_progress(image_stream, out_stream, image_size, &ierror)) {
		g_propagate_error(error, ierror);
		return FALSE;
	}

...
.. note:: This feature requires support from the bootloader. Currently, this is the case for `barebox`.
...
This reduces the effort required to maintain bootloader and OS consistency.
...
This case is handled by RAUC in order to avoid inconsistent results and unbootable systems.
During the update process, RAUC compares the bootloader update target with
the currently activated bootloader.
If the bootloader currently active is also the one intended for the bootloader target slot,
it will be copied to the slot that is inactive, which is then activated by RAUC.
The update procedure will then continue.

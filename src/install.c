...
gboolean do_install_bundle(RaucInstallArgs *args, GError **error)
{
	...
	if (bundle->manifest && bundle->manifest->bundle_format == R_MANIFEST_FORMAT_CRYPT && !bundle->was_enveloped) {
		g_set_error(error, R_INSTALL_ERROR, R_INSTALL_ERROR_REJECTED, "Refused to install unencrypted crypt bundles");
		res = FALSE;
		goto out;
	}
